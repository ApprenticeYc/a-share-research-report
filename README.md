# 量化分析系统 · 公开研究工作台

公开首页汇总市场、财务排雷、公司、ETF、债基与偏债混合、公告、方法论、版本和自动化；R2保留研发槽位。私人资产明细、账户凭据、本机路径和其他敏感信息不进入公开页面。

- `index.html`：完整能力入口与公开运行状态
- `a-share-report.html`：A股市场全面认知报告
- `market-temperature.html`：A股每日市场状态
- `financial-risk.html`：全市场财务排雷
- `company-research.html`：个股综合分析库
- `etf-research.html`：ETF 季报、持有公司与业务分类穿透
- `disclosures.html`：巨潮与披露更新
- `macro.html`：宏观研究边界与输入
- `research-history.html`：研究版本与回溯
- `system-status.html`：自动化与发布门禁
- `companies/600900-yangtze-power/index.html`：长江电力综合研究
- `companies/09888-baidu/index.html`：百度集团综合研究
- `companies/09618-jd/index.html`：京东集团综合研究
- `companies/pdd-holdings/index.html`：PDD Holdings 综合研究
- `companies/02015-li-auto/index.html`：理想汽车综合研究
- `companies/09868-xpeng/index.html`：小鹏汽车综合研究
- `lynch-methodology.html`：彼得林奇投研方法论
